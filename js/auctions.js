...
const names = ["Tom Brady", "Madonna", "Ben Simmons", "Justin Bieber", "Jimmy Fallon",
               "Post Malone", "Neymar Jr", "Kevin Hart", "Paris Hilton", "Snoop Dogg",
               "Von Miller", "Lil Baby"];
const descriptions = ["former American football quarterback","American singer",
                      "Australian professional basketball player","Canadian singe",
                      "American stand-up comedian","Singer and song writer",
                      "Soccer star","Celebrity comedian and actor",
                      "American personality and actress","American rapper and NFT collector",
                      "Los Angeles Rams Linebacker","American Rapper "];

// Random auction information
function generateRandomAuctions() { // Load input data 
  // Random cat images
 // document.querySelectorAll(".card > img").forEach(img => {
    // img.src = "https://cataas.com/cat/cute?random=" + Math.random();
    document.querySelectorAll(".card > img").forEach((img, index) => {
    img.src = imagePath + imageNames[index];
 ...
 /* $.getJSON(
  ...
  document.querySelectorAll(".card-body").forEach((auction, idx) => {
   // const randomIndex = Math.floor(Math.random() * names.length);
    const randomName = names[idx];
    auction.querySelector("h5").innerHTML = randomName;
    titles.push(randomName);
  });
 ...
 /* $.getJSON(
  ...
  document.querySelectorAll(".card-body").forEach((auction, idx) => {
    // const randomIndex = Math.floor(Math.random() * names.length);
     ...
     auction.querySelector("p").innerHTML = randomDescription;
     titles.push(randomDescription);
   });
